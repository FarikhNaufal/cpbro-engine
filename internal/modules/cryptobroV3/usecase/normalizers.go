...
func NormalizeLatestResultForFrontend(res *entity.LatestResult) dto.LatestResultResponse {
	...
	out.ActivePolicyLongMode = res.ActivePolicyLongMode
	out.ActivePolicyShortMode = res.ActivePolicyShortMode
	out.ActivePolicyRequireAIConfidence = res.ActivePolicyRequireAIConfidence
	out.ActivePolicyRequireFreshEntry = res.ActivePolicyRequireFreshEntry
	out.ActivePolicyAllowedPlaybooks = append([]string(nil), res.ActivePolicyAllowedPlaybooks...)
	...
	out.BootstrapTickerSource = res.BootstrapTickerSource
	out.BootstrapTickerCacheAgeSeconds = res.BootstrapTickerCacheAgeSeconds
	out.BootstrapFundingSource = res.BootstrapFundingSource
	out.BootstrapFundingCacheAgeSeconds = res.BootstrapFundingCacheAgeSeconds
...
func NormalizeJournalForFrontend(items []SignalJournal, limit int, offset int, filters dto.JournalFilters) dto.JournalResponse {
	...
	for _, item := range items {
		...
		outItems = append(outItems, dto.SignalJournalResponse{
			...
			PolicyLongMode:            item.PolicyLongMode,
			PolicyShortMode:           item.PolicyShortMode,
			PolicyRequireAIConfidence: item.PolicyRequireAIConfidence,
			PolicyRequireFreshEntry:   item.PolicyRequireFreshEntry,
			PolicyAllowedPlaybooks:    append([]string(nil), item.PolicyAllowedPlaybooks...),
...
func NormalizeDecisionAuditForFrontend(audits []DecisionAudit, limit, offset int, filters dto.DecisionAuditFilters) dto.DecisionAuditResponse {
	...
	for _, item := range audits {
		...
		outItems = append(outItems, dto.DecisionAuditRow{
			...
			PolicyAllowedPlaybooks:          append([]string(nil), item.PolicyAllowedPlaybooks...),
			PolicyReason:                    item.PolicyReason,
			CompressionLowVolFallbackActive: item.CompressionLowVolFallbackActive,
			BootstrapTickerSource:           item.BootstrapTickerSource,
			BootstrapTickerCacheAgeSeconds:  item.BootstrapTickerCacheAgeSeconds,
			BootstrapFundingSource:          item.BootstrapFundingSource,
			BootstrapFundingCacheAgeSeconds: item.BootstrapFundingCacheAgeSeconds,
			ThresholdProfileSummary:         item.ThresholdProfileSummary,
			BreakoutLevel:                   item.BreakoutLevel,
			RetestTouches:                   item.RetestTouches,
			RetestHold:                      item.RetestHold,
